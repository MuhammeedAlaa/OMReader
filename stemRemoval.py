...
def stemRemoval(img, staffLineSpacing):
  ...
  verticalProjection= (np.sum((255 - outputImg)/255, axis=0)).astype('uint64')
      ...
      if outputImg[y, stem] != 0:
        for j in range(1, round(threshold/2)):
          ...
          if stem - j > -1 and outputImg[y, stem - j] == 0:
            stem = stem - j
            break
      horizontalThresholdResult = testHorizontalThreshold(outputImg, stem, y, maxStemWidth)
      if(horizontalThresholdResult[0]):
        rr, cc = line(y, horizontalThresholdResult[1], y, horizontalThresholdResult[2])
        outputImg[rr, cc] = 255
...
def rle(bits):
  ...
  if n == 0: 
    ...
    y = np.array(bits[1:] != bits[:-1])     # pairwise unequal (string safe)
